...
# Slightly adapted from https://github.com/cleversonahum/dtmf-generator
...
class DtmfGenerator:
    ...
    def __dtmf_function(
        ...
        delay_samples = np.sin(2 * np.pi * 0 * time_delay, dtype=np.float16)
        return np.append(tone_samples.astype(np.int16), delay_samples.astype(np.int16))
...
def main():
    try:
        ...
        w.setparams((1, 2, args.samplefrequency, 0, "NONE", "not compressed"))
